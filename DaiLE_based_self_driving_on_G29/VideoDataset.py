...
class VideoDataset(Dataset):
    def __init__(self, data, transform, timesteps):      
        ...
        self.windows_per_minibatch = 600-self.timesteps
        
        directory_structure = []
        
        for directory in self.data:
            cur_img_path = sorted(glob.glob(directory+"\\img\\*.jpg"), key=len)[:600]
            cur_label_path = sorted(glob.glob(directory+"\\input\\*.npy"), key=len)[:600]
            cur_tel_path = sorted(glob.glob(directory+"\\telemetry\\*.npy"), key=len)[:600]
            
            if len(cur_img_path) >= 600:
