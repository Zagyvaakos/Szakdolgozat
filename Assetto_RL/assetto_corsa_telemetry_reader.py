#Code from https://github.com/dzosz/OpenRacingHUD/blob/master/scripts/assetto_corsa_telemetry_reader.py
...
if __name__ == '__main__':
    ...
    while True:
        ...
        tyresOut = assettoReader.getData().get('numberOfTyresOut')  
        speed = assettoReader.getData().get('speed')
        carDamageFront = assettoReader.getData().get('carDamagefront')  
        vel1 = assettoReader.getData().get('velocity1')  
        vel2 = assettoReader.getData().get('velocity2')  
        vel3 = assettoReader.getData().get('velocity3')  
        print ('Assetto data:', throttle, ' ',brake, ' ',steering, ' ', tyresOut, ' ', speed ,' ', carDamageFront, ' ', vel1, ' ', vel2, ' ', vel3   )        
        # print(throttle, brake, steering, tyresOut)
        
        #ha van rá opció kézifék illetve sebességfokozat beiktatása 
        #jegyzeeet - numberOfTyresOut meghatározása a rewardnak, plusz reward system és jó lenne eljutni a következő modell felépítéséig